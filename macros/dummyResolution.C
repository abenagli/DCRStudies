...
void drawPlots()
{
  ...
  methods.push_back("dummyStep_1pe");
  methods.push_back("dummySiPM_1pe");
  
  TF1* f_30ps = new TF1("f_30ps","30.",0.,100000.);
  ...
  for(unsigned int thrIt = 0; thrIt < thrs.size(); ++thrIt)
  {
    ...
    TH1F* hPad = (TH1F*)( gPad->DrawFrame(1.,1.,100000.,100000.) );
    gPad -> SetLogx();
    gPad -> SetLogy();
    ...
    for(unsigned int nPhEIt = 0; nPhEIt < nPhEs.size(); ++nPhEIt)
    {
      ...
      TFile* inFile = TFile::Open(Form("plots/studyDCR_nPhE%05d_DCR000.000GHz_SPTR0.000ns_dummyStep_1pe_nToys1000.root",nPhE),"READ");
      ...
      if( h1_timeNthPhE->GetEntries() < 100 ) continue;

      g_timeNthPhE -> SetPoint(g_timeNthPhE->GetN(),nPhE,1000.*h1_timeNthPhE->GetRMS());
      g_timeNthPhE -> SetPointError(g_timeNthPhE->GetN()-1,0.,1000.*h1_timeNthPhE->GetRMSError());
      
      g_timeAvgNPhE -> SetPoint(g_timeAvgNPhE->GetN(),nPhE,1000.*h1_timeAvgNPhE->GetRMS());
    ...
    for(auto method: methods)
    {
      ...
      for(unsigned int nPhEIt = 0; nPhEIt < nPhEs.size(); ++nPhEIt)
      {
        ...
        TFile* inFile = TFile::Open(Form("plots/studyDCR_nPhE%05d_DCR000.000GHz_SPTR0.000ns_%s_nToys1000.root",nPhE,method.c_str()),"READ");
        
        TH1F* h1_timeLE = (TH1F*)( inFile->Get(Form("h1_timeLE_thr%04dPhE",thr)) );
        
        if( h1_timeLE->GetEntries() < 100 ) continue;
        
        g_timeLE[method] -> SetPoint(g_timeLE[method]->GetN(),nPhE,1000.*h1_timeLE->GetRMS());
    ...
    for(auto method: methods)
    {
      ...
      TLatex* latex_LE = new TLatex(0.20,0.25+0.05*methodIt,Form("%s: f(N_{pe}) = %.1e / N_{pe}^{%.2f} + %.1f",
                                                                 method.c_str(),fitFunc_LE->GetParameter(0),fitFunc_LE->GetParameter(1),fitFunc_LE->GetParameter(2)));
      latex_LE -> SetNDC();
