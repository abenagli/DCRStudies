...
int main(int argc, char** argv)
{
  ...
  for(auto nPhE : nPhEs)
  {
    for(auto DCR : DCRs)
    {
      ...
      inFileNames[dummy] = Form("plots/laser/studyDCR_nPhE%05d_DCR%07.3fGHz_SPTR%.3fns_%s_CFD00.400ns_nToys%d.root",nPhE,DCR,SPTR,inFileName_1pe.c_str(),nToys);
    }
  }
  
  
  //--- define plot dir
  std::string plotDir = Form("plots/laser/DCRPlots_SPTR%.3fns_%s",SPTR,inFileName_1pe.c_str());
